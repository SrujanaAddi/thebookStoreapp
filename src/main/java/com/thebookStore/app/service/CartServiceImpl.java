...
public class CartServiceImpl implements CartService{
	...
	public Cart getCartById(Integer Id) throws CartException {
		Optional<Cart> optCarts = cartRepository.findById(Id);
		if(optCarts.isEmpty()) {
			throw new CartException("Cart id not found:" +Id);
		}
		return optCarts.get() ;
	}
	...
	public Books removeBookfromCartByIds(Integer bookId, Integer cartId) throws CartException{
		...
		Map<Integer, Books> books = foundCart.getBooksInCart();
		books.remove(bookId);
	...
	public Cart addBookToCartByUserId(Integer bookId, Integer userId)
		...
		if(cart == null) {
			cart = new Cart();
			cart.getBooksInCart().put(bookId, book);
